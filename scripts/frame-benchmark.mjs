...
const feedbackStress = process.env.EVE_BENCH_FEEDBACK === '1';
const output = process.env.EVE_FRAME_OUTPUT || 'tests/frame-benchmark-results.json';
...
for (const [build,url] of [['current',current],...(previous ? [['previous',previous]] : [])]) {
  for (const [layout,width,height] of [['desktop',1440,1000],['mobile-layout',390,844]]) {
    ...
    await page.evaluate(({build,feedbackStress}) => {
      paused=true;clearTimeout(timer);state.current=0;state.phase='roll';
      if(build==='current' && window.EveSound) {
        if(!feedbackStress && soundEffects.status.enabled) document.querySelector('#sound-toggle').click();
        if(feedbackStress) {
          for(let i=0;i<10000;i++) S.log(state,`${state.players[i%4].name} benchmark history ${i}.`,state.players[i%4].color,{player:i%4});
          feedback.reset(state); render();
        }
      } else audioEnabled=false;
    },{build,feedbackStress});
    for (const heavy of [false,true]) {
      ...
      for (const moving of [false,true]) {
        const sample = await page.evaluate(async ({moving,build,feedbackStress}) => {
          ...
          if(moving) (async()=>{while(running){const t=performance.now();await animateAction(()=>{
            state.players[0].position=(state.players[0].position+8)%40;state.lastRoll=[4,4];
            if(feedbackStress && build==='current') {S.log(state,'Benchmark roll 4 + 4.',undefined,{kind:'roll',player:0});S.log(state,'Benchmark rent receipt.',undefined,{kind:'rent',player:1,to:0,index:1,amount:50});}
            return true;
...
await browser.close();
await writeFile(output,JSON.stringify({recorded:new Date().toISOString(),feedbackStress,environment:'Headless Chromium; mobile is viewport emulation, not physical phone hardware. rAF frame pacing includes browser rendering; CPU draw time is the last submitted draw, including any one-time cache rebuild, and excludes asynchronous GPU execution. Model rendering is intentionally capped at 30 Hz during movement and sleeps at rest; camera animation uses requestAnimationFrame. The previous mobile layout cropped the board instead of fitting it, so that comparison involves different visible content. Feedback stress mode adds 10,000 historical entries, enabled real EVE audio and synthetic roll/rent-notification events during movement; this is a rendering fixture, not a rules simulation.',results},null,2)+'\n');
